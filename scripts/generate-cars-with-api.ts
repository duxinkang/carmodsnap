...
function buildPrompt(car: typeof CAR_MODELS[0]): string {
  ...
  return `Professional automotive photography of ${car.name} (${typeDesc[car.type] || 'car'}), ${car.brand} brand, full body shot, complete vehicle view, studio lighting, 4K resolution, dark gradient background, 45-degree front side angle, complete body visible, no cropping, ultra high detail, photorealistic, commercial quality, sleek modern design`;
}
...
console.log(`\n🚗 开始使用 API 生成 ${CAR_MODELS.length} 款车型图片...\n`);
...
async function pollTaskStatus(taskId: string): Promise<string | null> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      console.log(`  进度：${task.status}... (${i + 1}/${maxAttempts})`);
      ...
      console.log(`  轮询出错：${error.message}，继续尝试...`);
      await new Promise(resolve => setTimeout(resolve, pollInterval));
    }
  }
...
async function generateCarImages() {
  ...
  for (let i = 0; i < CAR_MODELS.length; i++) {
    ...
    try {
      // 调用生图 API
      const createResponse = await fetch('http://localhost:3000/api/ai/generate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!createResponse.ok) {
        throw new Error(`API 响应失败：${createResponse.status}`);
      }
