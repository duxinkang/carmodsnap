...
def create_video_task(prompt, size="1280*720", duration=5):
    ...
    headers = {
        ...
        "X-DashScope-Async": "enable"  # 异步生成
    }
    
    payload = {
        "model": "wanx2.0-t2v",  # 万相 2.0 文生视频
        "input": {
            "prompt": prompt
        },
        "parameters": {
...
def main():
    ...
    # 测试提示词 - 360 度环绕汽车
    prompts = [
        "360 度环绕一辆改装汽车拍摄，专业产品展示视频，电影级灯光，平滑的圆形相机运动，4K 画质",
        "一辆跑车在摄影棚中，相机环绕车辆旋转 360 度，展示车辆细节，专业商业视频",
        "Car 360 degree orbit shot, professional product showcase, cinematic lighting, smooth camera movement"
    ]
    ...
    for i, prompt in enumerate(prompts, 1):
        ...
        if task_id:
            ...
            if video_url:
                print(f"\n🎬 测试 {i} 完成！视频已生成")
                # 可以选择下载视频
                # download_video(video_url, f"test_video_{i}.mp4")
            else:
                print(f"\n❌ 测试 {i} 失败")
